			...
			<center>
				...
				src="https://res.cloudinary.com/doaf7ybhd/image/upload/v1619613106/9C0DF1E9-B878-49EC-ADAA-6B2267675DE1_lsme67.png"
												...
												return(
													<div>
														<h6 key={comment._id} style={{float: "left"}}>
															<span style={{fontWeight: "500", float: "left"}}><a href={`/user/${comment.postedBy._id}`}>{comment.postedBy.name}</a>: </span> {comment.text}
														</h6>
									...
									<div className="createdAt">
										...
										<h6 style={{fontSize: "2vh", float: "left"}}>Oluşturma Tarihi: { item.createdAt !== undefined ? item.createdAt.substring(0,10) : "Yükleniyor..." } { item.createdAt !== undefined ? Number(item.createdAt.substring(11,13)) + 3 : ""}:{ item.createdAt !== undefined ? Number(item.createdAt.substring(14,16)) + 3 : ""}</h6>
				...
				}
				{
					data.length === 0 ?
						...
						<h5 style={{fontSize: "3.5vh"}}>Yükleniyor...</h5>
					</div>
